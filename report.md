...
| `/manager/invoices` | `generateInvoice()` | Generates invoices for a student via a **per-type amount grid** (Room Rent / Electricity / Food / Water / Other) — fill in only the types you want and pick a **shared due date**; empty boxes are skipped and the student gets **one** notification listing every new bill + the total. Invalid amounts flash a warning and are skipped; at least one amount is required. The page shows a **Summary by Type** section (count + sum per type, grand total), plus an **Invoices by Student** table — each student is **one expandable row** (first invoice ID, bill types, count, total) that expands to their individual invoices with Paid/Unpaid toggle and **Print**, and each row has a **Print statement** button (see below) |
...
| `/manager/invoices/print-student/<id>` | — | **Print a statement for one student** — a printable one-page summary (`invoice_statement_print.html`) listing every invoice that student has (No, type, due date, status, amount) plus their **total invoice count and total value** (with paid / unpaid split) — print one student at a time via the **Print statement** button on their row |
...
8. **Invoice print ownership** — the student print route (`/student/invoices/print/<id>`) verifies the invoice belongs to the logged-in student before rendering; manager routes are role-guarded. **Print-only**: the redundant PDF downloads were removed (both roles), so receipts and per-student statements are produced via the browser's Print / Save-as-PDF on the printable pages (`invoice_print.html`, `invoice_statement_print.html`) — no extra library needed.
