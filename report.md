...
2. Load the database (one time):
   - `hostel_management_schema.sql` creates the database and all 18 tables.
...
| `student6` … `student50` | `student123` | 50 students total (generated by `scripts/generate_seed.py`), with 2.5 months of daily attendance records |
...
| `/notifications` | Lists the logged-in user's notifications, newest first; unread ones are highlighted. **Filter chips** at the top filter the list in the browser — All / Unread and per-category chips (Complaints, Mess Off, Parcels, Visitors, Attendance, Invoices, Violations, Notices, ...) — each notification row carries a `data-category` attribute that the chips toggle |
...
|---|---|---|
| `/manager` | — | Dashboard: counts of students/rooms/complaints/invoices + open violations + pending mess-off requests + recent complaints & violations — recent lists toggle between **Complaints** and **Violations** via pill tabs, and a new **Attendance analytics** section shows **today's status donut** (Present / Absent / Leave) plus a **last-14-days trend chart** (Chart.js from CDN) |
...
| `/manager/rooms` | `viewRooms()` | Two **pill-tab views — Hostels and Rooms**. Shows each hostel (with its gender), each room and bed counts. Each room row **expands** (an extra `.room-detail` row) to list its occupants — student **name, student no, email and phone** (via a `GROUP_CONCAT` subquery) — not just a count |
...
| `/manager/attendance` | `recordStudentAttendance()` `recordStaffAttendance()` | Records attendance; `ON DUPLICATE KEY UPDATE` lets you overwrite the same (student, date) instead of erroring. Two modes: the main list (one row per student/staff per date) and a **per-person View/Edit** — pick a student or staff member from a dropdown to see **their** history (this was a bug fix: the table used to list every student's records regardless of who was selected) and change any day's status inline, with **their own donut + 14-day trend charts** |
...
| `/student/attendance` | `recordAttendance()` | **Geo-fenced attendance** — the browser fetches the student's GPS position and a free Leaflet/OSM map shows their hostel's radius circle plus their own (draggable) location marker with an inside/outside badge; the captured fix uses `enableHighAccuracy` + `maximumAge: 0` and shows a **±accuracy estimate** (with a dashed accuracy circle) so coarse desktop (IP/WiFi-based) fixes are visibly unreliable; `Present` is only recorded if the haversine distance to the hostel is within `radius_m` (Leave needs no location). The page also shows **personal stats** (total days, present/absent/leave counts) with **donut + 14-day trend charts** (Chart.js) |
...
| `/staff/attendance` | `recordAttendance()` | **Geo-fenced attendance** — staff pick their workplace hostel, the browser fetches their GPS position, and a Leaflet/OSM map shows the selected hostel's radius circle plus their own location marker with an inside/outside badge and the **±accuracy** of the fix; `Present` is only recorded inside that hostel's `radius_m` circle (haversine); Leave needs no location. Personal **stats cards + donut + 14-day trend charts** (Chart.js) as on the student page |
...
- `manager/` — dashboard + 16 feature pages (students, register student, rooms — with Hostel/Room tabs and expandable occupants, add hostel, add room, allocate room, complaints, invoices, attendance — with charts, mess menu, violations, visitors, staff, feedback, mess off, parcels).
...
- `css/style.css` — one stylesheet for the whole app. CSS variables at the top (`:root { --primary: ... }`) define the colour theme, and `.card`, `.table`, `.form-group`, `.btn`, `.badge`, `.flashes` classes are reused everywhere. Added for the new features: `.notif-*` (notification feed + bell badge), `.map-canvas` (Leaflet container), `.check-label`, `.tabs`/`.tab-btn`/`.tab-panel`/`.chip`/`.row-notify` (violations tabs + filter chips + inline notify), `.room-detail` (expandable room-occupant rows), `.chart-card`/`.chart-container` (responsive Chart.js cards — the canvas resizes fluidly down to phone widths), and a flex-column layout that **pins the footer to the bottom** of the viewport (`main{flex:1}`) so short pages don't leave it floating mid-screen.
- `js/main.js` — forms with `data-confirm="..."` show a browser confirm dialog before submitting (the "Delete student?" buttons). New: the **gender filter** — on the allocate page it hides rooms whose hostel gender doesn't match the selected student; the **rooms expand/collapse** toggle for occupant detail rows; and the **notification filter chips** which show/hide rows by `data-category`. Chart.js itself is loaded from a CDN in the templates (no bundled library).
...
- `users` (base / superclass) + `managers`, `staff`, `students` (subtype tables, each linked by a `user_id` FK) — this is **table-per-subtype inheritance**. `students` and `staff` each carry a unique human-readable number (`student_no` e.g. `STU-0001`, `staff_no` e.g. `STF-001`) that appears in lists and room-occupant details.
...
### `seed_data.sql`
Inserts demo records (54 users: 1 manager, 3 staff, 50 students — 33 male + 17 female; 2 hostels — Main Hostel (Male, 9 rooms) and Girls Hostel (Female, 4 rooms) with Dhaka-area GPS coords and a 50 m attendance radius; 38 students allocated to rooms, 12 unallocated; a full-week mess menu; 28 complaints, 25 feedback, 15 mess-off requests, 18 visitors, 20 parcels, 12 violations, 22 in/out records and **120 invoices** (2 monthly room-rent bills per student + ~1-in-3 gets an electricity bill)). The crown jewel is **2.5 months (2026-06-01 → 2026-08-11) of daily attendance** for every student (3,600 rows) and each staff member (216 rows) — weighted Present/Absent/Leave ~80/10/10 — which is what powers the dashboard and per-person analytics charts out of the box. Passwords are stored as **scrypt hashes**, never plain text.

**`scripts/generate_seed.py`** regenerates this file deterministically (fixed random seed → stable names, hashes, and room assignments). It builds the same dataset described above — student/staff numbers, hostels/rooms with varied capacities, menu, the daily-attendance generator, and the record pools — and writes `seed_data.sql`. After changing demo data, just re-run it to rebuild `seed_data.sql`, then re-import.
